...
# -*- coding: utf-8 -*-
"""
Gerenciador do Dashboard Master
Script para configurar e gerenciar o sistema de menus do Dashboard Master
"""
            ...
            if item.get("type") == "group":
                ...
                if "children" in item:
                    for child in item["children"]:
                        print(f"   └── 📊 {child['title']} → {child.get('file', 'N/A')} (ID: {child['id']})")
                else:
                    print("   └── (vazio)")
            else:
                icon = item.get('icon', '📄')
                file_info = f" → {item.get('file', 'N/A')}" if item.get('file') else ""
                print(f"{icon} {item['title']}{file_info} (ID: {item['id']})")
    
    def generate_html(self, output_file="dashboard_master_generated.html"):
        """Gera o arquivo HTML do dashboard com a configuração atual"""
...
<body>
    <div class="dashboard-container">
        <div class="sidebar" id="sidebar">
            <div class="sidebar-header">
                ...
                <div class="subtitle">Opinião Informação Estratégica</div>
            </div>
        ...
        <div class="main-content">
            <header class="main-header">
                ...
                <p class="subtitle" id="mainSubtitle">Selecione uma análise no menu lateral</p>
            ...
            <div class="welcome" id="welcomeState">
                ...
                <p>Dashboard gerado automaticamente em {generated_at}</p>
                ...
                <div style="background: var(--hover); padding: 20px; border-radius: 8px; margin-top: 20px;">
                    <h3 style="color: var(--primary); margin-bottom: 10px;">Instruções:</h3>
                    <ol style="text-align: left; color: var(--text); line-height: 1.8;">
                        <li>Navegue pelo menu lateral para acessar as análises</li>
                        <li>Use o botão ⟵ para recolher/expandir a barra lateral</li>
                        <li>Para adicionar novas análises, use o gerenciador Python</li>
    ...
    <script>
            ...
            setTimeout(() => {{
                ...
                frame.onerror = () => {{
                    ...
                    welcome.innerHTML = `
                        ...
                        <p>O arquivo "${{item.file}}" não foi encontrado. Verifique se está na mesma pasta.</p>
                        <div class="info-box">
                            <strong>Dica:</strong> Certifique-se de que o arquivo HTML foi gerado pelo script SPSS 
                            e está na mesma pasta que este dashboard.
                        </div>
                    `;
        ...
        function showWelcome() {{
            ...
            subtitle.textContent = 'Selecione uma análise no menu lateral';
        }}
        ...
        stats_text = f"{total_groups} grupos, {total_files} análises individuais, {total_subitems} subitens"
...
def main():
    print("🚀 GERENCIADOR DO DASHBOARD MASTER")
    print("=" * 50)
    ...
    while True:
        ...
        if choice == "1":
            ...
            filename = input("Nome do arquivo HTML: ").strip()
            if not filename:
                print("❌ Nome do arquivo é obrigatório")
                continue
            
            # Adiciona .html se não tiver
            ...
            print("\n🌐 GERAR HTML")
            output_name = input("Nome do arquivo (deixe vazio para 'dashboard_master_generated.html'): ").strip()
            if not output_name:
                output_name = "dashboard_master_generated.html"
            
            if not output_name.endswith('.html'):
                output_name += '.html'
                
            manager.generate_html(output_name)
        
        elif choice == "6":
            print("\n📤 EXPORTAR CONFIGURAÇÃO")
            export_name = input("Nome do arquivo JSON (deixe vazio para 'dashboard_config.json'): ").strip()
            if not export_name:
                export_name = "dashboard_config_export.json"
