...
def _detect_skip_logic(vars_meta: List[dict], records: List[dict]) -> None:
    ...
    for qy_idx, vm_y in enumerate(vars_meta):
        ...
        # ── PASSO 1: tentar MR-option como fonte (mais específico) ────────
        # Para cada Qx MR anterior, testa cada opção O da MR como filtro binário
        # (marcou_O / não_marcou_O). Se algum O dividir cleanly entre included
        # (alta resposta de Qy) e excluded (baixa), assume Qy ← Qx[O].
        mr_match = None
        for qx_idx in range(qy_idx):
            vm_x = vars_meta[qx_idx]
            if (vm_x.get("var_type") or vm_x.get("type")) != "multiple_response":
                continue
            qx_name = vm_x["name"]
            # coletar opções únicas que aparecem nos records
            opt_set = set()
            for r in records:
                v = r.get(qx_name)
                if v and isinstance(v, list):
                    opt_set.update(v)
            for opt in opt_set:
                has_o, has_o_resp = 0, 0
                for i, rec in enumerate(records):
                    v = rec.get(qx_name)
                    if isinstance(v, list) and opt in v:
                        has_o += 1
                        if i in resp_qy:
                            has_o_resp += 1
                not_o = total_records - has_o
                not_o_resp = n_qy - has_o_resp
                # Para MR-option o threshold mínimo é mais permissivo: a regra
                # "só quem marcou X responde" é uma instrução de questionário,
                # não inferência estatística. n>=2 já é suficiente.
                if has_o < 2 or not_o == 0:
                    continue
                rate_in  = has_o_resp / has_o
                rate_out = not_o_resp / not_o
                if rate_in >= HIGH and rate_out <= LOW:
                    coverage = round(has_o_resp / n_qy * 100, 1) if n_qy > 0 else 0.0
                    mr_match = {
                        "source_var":   qx_name,
                    ...
                    print(f"🔗 Skip logic detectado: {qy_name} ← {qx_name}[{opt}] "
                          f"cobertura={coverage}% (MR option)")
                    break
            if mr_match:
                break
        if mr_match:
            vm_y["skip_logic"] = mr_match
            continue  # MR option encontrado — não fazer fallback single-type

        # ── PASSO 2: fallback para Qx single-type (categórica simples) ─────
        for qx_idx in range(qy_idx):
            vm_x = vars_meta[qx_idx]
            qx_name = vm_x["name"]
...
# Paleta default (Opinião / teal). As chaves aqui podem ser sobrescritas
# em dashboard_overlay_config.json via:
#   "palette": { "primary": "#C5D92E", "primary_dark": "#5c6a00", ... }
# Apenas as chaves presentes no JSON são trocadas; as demais mantêm o default.
# Verde/vermelho são SEMÂNTICOS (NPS+, NPS−) — só troque se for intencional.
_DEFAULT_PALETTE = {
    'primary':      '#7BAFC0',
    'primary_dark': '#4d8a9e',
    'teal_light':   '#EDF5F7',
...
def _apply_gate(html: str, password: str, storage_key: str,
                primary_color: str = '#7BAFC0',
                project_label: str = 'Painel de Resultados') -> str:
    """Adiciona gate de senha SHA-256 com SSO via localStorage.

    SSO: lê/escreve a mesma chave no localStorage que outras páginas do projeto
    usem (ex.: APN com `nro_apn_auth`). Se a chave já vale '1' (autenticado lá),
    o gate é pulado. Bypass por URL: ?auth=1 — espelha o comportamento da APN.
    """
    if not password or not storage_key:
        return html
    import hashlib
    pwd_hash = hashlib.sha256(password.encode('utf-8')).hexdigest()
    opiniao_logo = 'https://grupoopiniao.inf.br/wp-content/uploads/2025/05/OPINIAO_marca_F01-300x196.png'

    gate_css = (
        '<style id="nro-gate-style">'
        '#gate{position:fixed;inset:0;z-index:2147483646;display:flex;align-items:center;justify-content:center;'
        'background:linear-gradient(135deg,#0d1117,#1e2a38);'
        'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;}'
        '#gate-box{background:#fff;border-radius:18px;padding:40px 36px;width:340px;text-align:center;'
        'box-shadow:0 30px 80px rgba(0,0,0,.5);}'
        '#gate-opiniao{margin-bottom:20px;}'
        '#gate-opiniao img{height:38px;}'
        '#gate-box h2{font-size:17px;font-weight:800;color:#0d1117;margin:0 0 6px;}'
        '#gate-box p{font-size:12.5px;color:#6b7280;margin:0 0 22px;line-height:1.5;}'
        '#gate-pw{width:100%;padding:12px 16px;border:1.5px solid #e5e7eb;border-radius:10px;'
        'font-size:14px;font-family:inherit;outline:none;transition:border-color .15s;'
        'box-sizing:border-box;}'
        f'#gate-pw:focus{{border-color:{primary_color};}}'
        '#gate-btn{width:100%;padding:13px;background:#0d1117;color:#fff;border:none;'
        'border-radius:10px;font-weight:800;cursor:pointer;margin-top:10px;font-size:13.5px;'
        'letter-spacing:.05em;text-transform:uppercase;font-family:inherit;}'
        '#gate-btn:hover{background:#1e2a38;}'
        '#gate-err{color:#dc2626;font-size:12px;min-height:18px;margin-top:6px;}'
        '</style>'
    )
    gate_html = (
        '<div id="gate" style="display:none">'
        '<div id="gate-box">'
        f'<div id="gate-opiniao"><img src="{opiniao_logo}" alt="Opinião"/></div>'
        f'<h2>{project_label}</h2>'
        '<p>Acesso restrito.<br/>Digite a senha para continuar.</p>'
        '<input id="gate-pw" type="password" placeholder="Senha" '
        "onkeydown=\"if(event.key==='Enter')gateCheck()\"/>"
        '<div id="gate-err"></div>'
        '<button id="gate-btn" onclick="gateCheck()">Acessar</button>'
        '</div></div>'
    )
    # JS do gate (SHA-256 inline, SSO compatível com APN). Usa string crua pra
    # não interpolar nada além de HASH e KEY.
    gate_js = (
        '<script id="nro-gate-script">(function(){'
        f"var HASH='{pwd_hash}';var KEY='{storage_key}';"
        'function sha256(str){function rr(n,d){return(n>>>d)|(n<<(32-d));}'
        ...
        'var msg=unescape(encodeURIComponent(str));var bytes=[];'
        'for(var i=0;i<msg.length;i++)bytes.push(msg.charCodeAt(i));bytes.push(0x80);'
        'while(bytes.length%64!==56)bytes.push(0);var L=msg.length*8;'
        'for(var i=7;i>=0;i--)bytes.push((L/Math.pow(2,i*8))&0xff);'
        'for(var chunk=0;chunk<bytes.length;chunk+=64){var w=[];'
        'for(var i=0;i<16;i++)w[i]=(bytes[chunk+i*4]<<24)|(bytes[chunk+i*4+1]<<16)|(bytes[chunk+i*4+2]<<8)|bytes[chunk+i*4+3];'
        'for(var i=16;i<64;i++){var s0=rr(w[i-15],7)^rr(w[i-15],18)^(w[i-15]>>>3);'
        'var s1=rr(w[i-2],17)^rr(w[i-2],19)^(w[i-2]>>>10);w[i]=(w[i-16]+s0+w[i-7]+s1)>>>0;}'
        'var a=H[0],b=H[1],c=H[2],d=H[3],e=H[4],f=H[5],g=H[6],h2=H[7];'
        'for(var i=0;i<64;i++){var S1=rr(e,6)^rr(e,11)^rr(e,25);var ch=(e&f)^(~e&g);'
        'var temp1=(h2+S1+ch+K[i]+w[i])>>>0;var S0=rr(a,2)^rr(a,13)^rr(a,22);'
        'var maj=(a&b)^(a&c)^(b&c);var temp2=(S0+maj)>>>0;'
        'h2=g;g=f;f=e;e=(d+temp1)>>>0;d=c;c=b;b=a;a=(temp1+temp2)>>>0;}'
        ...
        "if(params.get('auth')==='1'||localStorage.getItem(KEY)==='1'){"
        "localStorage.setItem(KEY,'1');unlock();return;}"
        "var gEl=document.getElementById('gate');if(gEl)gEl.style.display='flex';"
        "setTimeout(function(){var pw=document.getElementById('gate-pw');if(pw)pw.focus();},120);"
        'window.gateCheck=function(){'
        "var val=document.getElementById('gate-pw').value;"
        "var err=document.getElementById('gate-err');"
        "if(!val){err.textContent='Digite a senha.';return;}"
        "if(sha256(val)===HASH){localStorage.setItem(KEY,'1');unlock();}"
    ...
    html = html.replace('</head>', gate_css + '</head>', 1)
    # Insere gate HTML logo após <body...>
    import re as _re
    html = _re.sub(r'(<body[^>]*>)', lambda m: m.group(1) + gate_html, html, count=1)
    html = html.replace('</body>', gate_js + '</body>', 1)
    return html


def _apply_palette(html: str, palette: dict) -> str:
    """Substitui hex defaults pelos valores em `palette`. No-op se vazio.

    Suporta também `palette["header_dark"] = True` (opt-in) para aplicar
    o padrão da apresentação NRO (topbar #0d1117 + borda lima + tag-pill).
    """
    if not palette:
        return html
    # 1. Substituições por chave (hex global)
    for key, default_hex in _DEFAULT_PALETTE.items():
        new_hex = palette.get(key)
        if not new_hex or new_hex == default_hex:
            continue
        for variant in {default_hex, default_hex.lower(), default_hex.upper()}:
            html = html.replace(variant, new_hex)

    # 2. Header escuro (opt-in via "header_dark": true)
    if palette.get('header_dark'):
        p  = palette.get('primary',      _DEFAULT_PALETTE['primary'])
        p2 = palette.get('primary_2',    p)  # opcional: 2ª cor para degradê das bordas
        pd = palette.get('primary_dark', _DEFAULT_PALETTE['primary_dark'])
        tl = palette.get('teal_light',   _DEFAULT_PALETTE['teal_light'])
        # rgba derivado do primary (10%/30% opacidade) para fundos translúcidos
        _h = p.lstrip('#')
        r, g, b = (int(_h[0:2], 16), int(_h[2:4], 16), int(_h[4:6], 16))
        rgba_10 = f'rgba({r},{g},{b},.10)'
        rgba_30 = f'rgba({r},{g},{b},.30)'
        # Gradiente lima (border-image-like via background-clip)
        # Quando primary_2 == primary, vira solid; senão degradê primary→primary_2.
        grad_lima = f'linear-gradient(90deg,{p},{p2})'

        # 2.1 Topbar e filter panel: white -> dark com degradê na borda
        html = html.replace(
            f'background:white;border-bottom:2px solid {p}',
            f'background:linear-gradient(#0d1117,#0d1117) padding-box,'
            f'{grad_lima} border-box;border-bottom:2px solid transparent',
        )
        # 2.2 Tag "Painel de Resultados" estilo APN (pill lima translúcido)
        html = html.replace(
            f'<span style="font-size:12px;font-weight:600;color:{pd};letter-spacing:.01em;">Painel de Resultados</span>',
            f'<span style="font-size:10px;font-weight:800;letter-spacing:.13em;text-transform:uppercase;color:{p};background:{rgba_10};padding:3px 12px;border-radius:20px;">Painel de Resultados</span>',
        )
        # 2.3 Botão prev (seta esquerda) — fundo lima translúcido
        html = html.replace(
            f'background:{tl};border:0.5px solid #b8d9e3;color:{pd}',
            f'background:{rgba_10};border:0.5px solid {rgba_30};color:{p}',
        )
        # 2.4 Botão filter (estado idle) — translúcido sobre dark
        html = html.replace(
            'background:#f4f6f8;border:0.5px solid #dde5ea;color:#888',
            'background:rgba(255,255,255,.06);border:0.5px solid rgba(255,255,255,.15);color:#e2e8f0',
        )
        # 2.5 Barra de filtros principal (.filters-container) — injeta override CSS
        # antes do </head> para escurecer e dar acentos lima estilo APN.
        dark_filterbar_css = (
            '<style id="nro-dark-filterbar">'
            '.filters-container{'
                'background:'
                    'linear-gradient(#0d1117,#0d1117) padding-box,'
                    f'{grad_lima} border-box !important;'
                'border-bottom:2px solid transparent !important;'
                'box-shadow:0 1px 4px rgba(0,0,0,0.35) !important;'
            '}'
            '.filter-bar-label,.filter-col-label{color:#e2e8f0 !important;}'
            '.filter-bar-label{border-right-color:rgba(255,255,255,.15) !important;}'
            '.filter-col-select{'
                'background:rgba(255,255,255,.06) !important;'
                'color:#e2e8f0 !important;'
                'border-color:rgba(255,255,255,.18) !important;'
            '}'
            '.filter-col-select option{background:#0d1117;color:#e2e8f0;}'
            f'.filter-col-select:focus{{border-color:{p} !important;}}'
            '.icon-btn{color:#cbd5e1 !important;}'
            '.icon-btn:hover{background:rgba(255,255,255,.08) !important;}'
            f'.icon-btn.present{{color:{p} !important;}}'
            f'.icon-btn.present:hover{{background:{rgba_10} !important;color:{p} !important;}}'
            '.icon-btn.apply{color:#86efac !important;}'
            '.icon-btn.apply:hover{background:rgba(134,239,172,0.10) !important;color:#86efac !important;}'
            '.icon-btn.clear{color:#fca5a5 !important;}'
            '.icon-btn.clear:hover{background:rgba(252,165,165,0.10) !important;color:#fca5a5 !important;}'
            '.icon-btn.excel{color:#86efac !important;}'
            '.icon-btn.excel:hover{background:rgba(134,239,172,0.10) !important;color:#86efac !important;}'
            '.icon-btn.pdf{color:#fca5a5 !important;}'
            '.icon-btn.pdf:hover{background:rgba(252,165,165,0.10) !important;color:#fca5a5 !important;}'
            '</style>'
        )
        html = html.replace('</head>', dark_filterbar_css + '</head>', 1)

        # 2.6 Moldura inferior (footer informacional) — opt-in via "footer_text"
        footer_text = palette.get('footer_text', '')
        if footer_text:
            opiniao_logo = 'https://grupoopiniao.inf.br/wp-content/uploads/2025/05/OPINIAO_marca_F01-300x196.png'
            footer_css = (
                '<style id="nro-dark-footer">'
                'body{padding-bottom:38px !important;}'
                '.nro-footer{'
                    'position:fixed;left:0;right:0;bottom:0;height:38px;'
                    'background:'
                        'linear-gradient(#0d1117,#0d1117) padding-box,'
                        f'{grad_lima} border-box;'
                    'border-top:2px solid transparent;'
                    'display:flex;align-items:center;justify-content:space-between;'
                    'padding:0 22px;z-index:2147483647;'
                    'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;'
                '}'
                '.nro-footer img{height:20px;filter:brightness(0) invert(1);opacity:.55;display:block;}'
                f'.nro-footer .meta{{font-size:10px;color:{rgba_30};letter-spacing:.13em;text-transform:uppercase;font-weight:700;}}'
                '@media print{.nro-footer{position:fixed;}}'
                '</style>'
            )
            html = html.replace('</head>', footer_css + '</head>', 1)
            footer_html = (
                '<div class="nro-footer">'
                f'<img src="{opiniao_logo}" alt="Opinião"/>'
                f'<div class="meta">{footer_text}</div>'
                '</div>'
            )
            html = html.replace('</body>', footer_html + '</body>', 1)

            # Footer dentro do overlay (modo apresentação) — flex child à prova de
            # qualquer stacking context. O overlay continua bottom:0; o footer
            # interno ocupa 38px do flex column, alinhando com a moldura externa.
            ov_footer_inline = (
                f'<img src="{opiniao_logo}" style="height:20px;filter:brightness(0) invert(1);opacity:.55;display:block;" alt="Opinião"/>'
                f'<div style="font-size:10px;color:{rgba_30};letter-spacing:.13em;text-transform:uppercase;font-weight:700;">{footer_text}</div>'
            )
            ov_footer_js = (
                "\n            const _ovFooter = document.createElement('div');"
                f"\n            _ovFooter.style.cssText = 'flex-shrink:0;height:38px;background:linear-gradient(#0d1117,#0d1117) padding-box,{grad_lima} border-box;border-top:2px solid transparent;display:flex;align-items:center;justify-content:space-between;padding:0 22px;font-family:-apple-system,BlinkMacSystemFont,sans-serif;';"
                f"\n            _ovFooter.innerHTML = `{ov_footer_inline}`;"
                "\n            overlay.appendChild(_ovFooter);"
                "\n"
            )
            html = html.replace(
                'overlay.appendChild(slideArea);',
                'overlay.appendChild(slideArea);' + ov_footer_js,
                1,
            )

    # 3. Fonte customizada (opt-in via "font_family" — Google Fonts)
    #    Ex.: "font_family": "Montserrat"  → carrega de fonts.googleapis.com
    #    Para outra fonte/CDN: passar "font_url" explícito.
    font_family = palette.get('font_family')
    if font_family:
        font_weights = palette.get('font_weights', '400;500;600;700;800')
        font_url = palette.get('font_url')
        if not font_url:
            _name = font_family.replace(' ', '+')
            font_url = (
                f'https://fonts.googleapis.com/css2?'
                f'family={_name}:wght@{font_weights}&display=swap'
            )
        font_inject = (
            f'<link href="{font_url}" rel="stylesheet"/>'
            '<style id="nro-font">'
            f'body,body *{{font-family:"{font_family}",system-ui,-apple-system,sans-serif !important;}}'
            '</style>'
        )
...
def render_html_with_working_filters(file_source: str, created_at: str, client_name: str,
    ...
    # Ler client_logo, description, palette e gate do dashboard_overlay_config.json (mesma pasta)
    client_logo = ''
    ...
    _palette = {}                    # Customização de cores por cliente (opcional)
    _gate_pw = ''                    # Senha do gate (opcional; ativa o gate se setada junto com storage_key)
    _gate_key = ''                   # Chave de localStorage para SSO entre páginas do projeto
    _project_label = 'Painel de Resultados'
    import os
    _crosstab_js_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "crosstab.js")
    try:
        with open(_crosstab_js_path, "r", encoding="utf-8") as _f:
            crosstab_js = _f.read()
    except Exception as _e:
        crosstab_js = "/* crosstab.js não encontrado: %s */" % _e
    config_candidates = [
    ...
    for cfg_path in config_candidates:
        if os.path.exists(cfg_path):
            try:
                ...
                _palette = _cfg.get('palette', {}) or {}
                _gate_pw = _cfg.get('password', '') or ''
                _gate_key = _cfg.get('gate_storage_key', '') or ''
                _project_label = _cfg.get('welcome_title') or _cfg.get('title') or 'Painel de Resultados'
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .view-tabs{{display:flex;gap:6px;justify-content:center;margin:14px auto 0;max-width:1180px;padding:0 22px}}
        .view-tab{{border:0;background:var(--teal-light,#EDF5F7);color:var(--primary-dark,#4d8a9e);
          font-weight:600;padding:8px 18px;border-radius:8px;cursor:pointer;font-size:13px}}
        .view-tab.active{{background:var(--primary,#7BAFC0);color:#fff}}
        .xt-banner{{max-width:1180px;margin:14px auto;padding:12px 18px;background:#fff;border:1px solid #dfe8ec;
          border-radius:12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}}
        .xt-banner-lbl{{font-weight:700;color:var(--primary-dark,#4d8a9e);font-size:13px}}
        .xt-banner-hint{{margin-left:auto;color:#6b7c86;font-size:12px;font-style:italic}}
        .xt-chip{{display:inline-flex;align-items:center;gap:7px;background:var(--teal-light,#EDF5F7);
          color:var(--primary-dark,#4d8a9e);border:1px solid #cfe2e8;border-radius:20px;padding:6px 12px;font-size:13px;font-weight:600}}
        .xt-chip-x{{cursor:pointer;color:#6b7c86;font-weight:700}}
        .xt-add{{border:1px dashed var(--primary,#7BAFC0);color:var(--primary-dark,#4d8a9e);background:#fff;
          border-radius:20px;padding:6px 10px;font-size:13px;cursor:pointer}}
        .xt-empty{{text-align:center;color:#6b7c86;padding:40px;font-size:14px}}
        .xt-card{{background:#fff;border:1px solid #dfe8ec;border-radius:12px;padding:18px;margin-bottom:24px}}
        .xt-title{{font-size:14px;margin:0 0 12px;color:#243845}}
        table.xt{{border-collapse:collapse;width:100%;font-size:13px}}
        table.xt th,table.xt td{{padding:7px 10px;text-align:center;border-bottom:1px solid #dfe8ec}}
        table.xt thead .xt-grp{{background:var(--primary,#7BAFC0);color:#fff;font-weight:700;border:1px solid #fff;font-size:12px}}
        table.xt thead .xt-grp.xt-total{{background:var(--primary-dark,#4d8a9e)}}
        table.xt thead .xt-cat{{background:var(--teal-light,#EDF5F7);color:var(--primary-dark,#4d8a9e);font-weight:600;font-size:12px}}
        table.xt td.xt-total-col{{background:#fafcfd;font-weight:600}}
        table.xt td.xt-rowlabel,table.xt th.xt-rowlabel{{text-align:left;font-weight:600;color:#243845}}
        table.xt tr.xt-base td{{color:#6b7c86;font-size:12px;border-top:2px solid #dfe8ec;border-bottom:0;font-weight:600}}
        .xt-smallbase{{font-style:italic;color:#9aa7ad}}
        .xt-up{{color:#3d8a62;font-weight:700}}
        .xt-down{{color:#B83B5C;font-weight:700}}
        .xt-legend{{font-size:11.5px;color:#6b7c86;margin-top:10px}}
    </style>
</head>
<body>
    ...
    <div class="view-tabs">
        <button id="tab-freq" class="view-tab active">Frequências</button>
        <button id="tab-cross" class="view-tab">Tabelas cruzadas</button>
    </div>

    <div id="view-frequencias">
        <div class="content" id="content">
            <!-- Conteúdo gerado dinamicamente -->
        </div>
    </div>

    <div id="view-crosstab" style="display:none">
        <div class="xt-toolbar" style="max-width:1180px;margin:0 auto 12px;padding:0 22px;display:flex;gap:10px">
            <button onclick="Crosstab.exportCrosstabExcel()" class="view-tab">⬇ Excel</button>
            <button onclick="Crosstab.exportCrosstabPDF()" class="view-tab">⬇ PDF</button>
        </div>
        <div id="crosstab-banner" class="xt-banner"></div>
        <div id="crosstab-content" class="content"></div>
    </div>
    ...
    function wrapLabel(label, maxLen) {{
        ...
        function applyFilters() {{
            ...
            if (window.refreshCrosstab) window.refreshCrosstab();
            updateFilterCounts();
...
// Excel exportado — sem popup
        ...
        function startPresentation() {{
            ...
            function renderSlide(idx) {{
                ...
                if (current === 0) {{
                    ...
                    const card = document.createElement('div');
                    card.style.cssText = 'background:white;border-radius:14px;width:100%;max-width:1200px;overflow:hidden;border:0.5px solid #dde5ea;box-shadow:0 4px 16px rgba(0,0,0,0.08);';

                    // Topo teal
                    const top = document.createElement('div');
                    top.style.cssText = 'background:#EDF5F7;padding:56px 72px 48px;';
                    const _nBruto = '<div><div style="font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px;">Respondentes</div><div style="font-size:22px;color:#444;font-weight:500;">' + RECORDS.length.toLocaleString('pt-BR') + '</div></div>';
                    const _nPondRaw = Math.round(RECORDS.reduce((s,r) => s + (r.__weight__||1), 0));
                    const _nPondHtml = Math.abs(_nPondRaw - RECORDS.length) > 0.5
                        ? '<div><div style="font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px;">N ponderado</div><div style="font-size:22px;color:#7BAFC0;font-weight:500;">' + _nPondRaw.toLocaleString('pt-BR') + '</div></div>'
                        : '';
                    ...
                    const _nFiltHtml = _hasFilter
                        ? '<div style="border-left:2px solid #7BAFC0;padding-left:24px;"><div style="font-size:11px;color:#4d8a9e;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px;font-weight:600;">N filtrado</div><div style="font-size:22px;color:#4d8a9e;font-weight:600;">' + _nFiltRaw.toLocaleString('pt-BR') + '</div></div>'
                        : '';
                    top.innerHTML =
                        '<div style="font-size:14px;font-weight:600;color:#7BAFC0;text-transform:uppercase;letter-spacing:.08em;margin-bottom:18px;">Painel de Resultados</div>' +
                        '<div style="font-size:42px;color:#1a1a1a;line-height:1.25;margin-bottom:48px;max-width:1000px;font-weight:600;letter-spacing:-.01em;">' + (DESCRIPTION || FILE_SOURCE) + '</div>' +
                        '<div style="display:flex;gap:56px;flex-wrap:wrap;align-items:flex-start;">' +
                          _nBruto + _nPondHtml + _nFiltHtml +
                          '<div><div style="font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px;">Perguntas</div><div style="font-size:22px;color:#444;font-weight:500;">' + VARS_META.length + '</div></div>' +
                          '<div><div style="font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px;">Período de coleta</div><div style="font-size:22px;color:#444;font-weight:500;">' + periodo + '</div></div>' +
                        '</div>';
                    card.appendChild(top);

                    // Rodapé com logos — criados via DOM para evitar problemas de aspas
                    const foot = document.createElement('div');
                    foot.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:22px 56px;border-top:0.5px solid rgba(123,175,192,0.2);';

                    const imgClient = document.createElement('img');
                    imgClient.src = CLIENT_LOGO;
                    imgClient.style.cssText = 'height:50px;object-fit:contain;';
                    imgClient.onerror = function() {{ this.style.display = 'none'; }};
                    foot.appendChild(imgClient);

                    const opDiv = document.createElement('div');
                    opDiv.style.cssText = 'display:flex;flex-direction:column;align-items:flex-end;gap:6px;';
                    const opLbl = document.createElement('div');
                    opLbl.style.cssText = 'font-size:13px;color:#bbb;';
                    opLbl.textContent = 'Desenvolvido por';
                    const imgOp = document.createElement('img');
                    imgOp.src = OPINIAO_LOGO;
                    imgOp.style.cssText = 'height:32px;opacity:.65;';
    ...
    _crosstab_block = ("<script>\n" + crosstab_js + "\n</script>\n"
                       + "<script>window.addEventListener('load', function(){ if(window.Crosstab) Crosstab.initCrosstab(); });</script>\n")
    _html = _html.replace("</body>", _crosstab_block + "</body>", 1)
    _html = _apply_palette(_html, _palette)
    _html = _apply_gate(
        _html, _gate_pw, _gate_key,
        primary_color=_palette.get('primary', '#7BAFC0'),
...
def run_cli() -> int:
    ...
    p.add_argument("--description", type=str, default="", help="Descrição (sobrescreve a do JSON)")
    p.add_argument("--period", type=str, default="", help="Período de coleta (texto livre, ex.: '27/03/2026 a 22/04/2026')")
