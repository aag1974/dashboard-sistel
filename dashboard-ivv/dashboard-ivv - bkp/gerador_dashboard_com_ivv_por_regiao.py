        ...
        function renderCrossTables(data) {
            ...
            cats.forEach(function(cat, index) {
                ...
                if (cat === 'gastos_pos_entrega') {
                    ...
                    // Estrutura especial para IVV por região
                    const table = document.createElement('table');
                    table.className = 'data-table';
                    const thead = document.createElement('thead');
                    ...
                    // Extrair quartos disponíveis dos dados
                    ...
                    // Calcular IVV por bairro e quartos
                    let totalVendas = 0;
                    let totalOfertas = 0;
                    ...
                    bairros.forEach(function(bairro) {
                        ...
                        // Primeira coluna: Nome do bairro
                        const tdBairro = document.createElement('td');
                        tdBairro.textContent = bairro;
                        tdBairro.style.fontWeight = '500';
                        row.appendChild(tdBairro);
                        
                        let bairroVendas = 0;
                        let bairroOfertas = 0;
                        
                        // Colunas por número de quartos
                        rooms.forEach(function(q) {
                            const td = document.createElement('td');
                            
                            // Buscar dados de vendas e ofertas para calcular IVV
                            const vendasData = filteredCrossData.filter(function(row) {
                                const bairroRow = row.BAIRRO || '';
                                let quartosRow = row.QTD_QUARTOS;
                                let qVal;
                                if (quartosRow === null || quartosRow === undefined || quartosRow === '') {
                                    qVal = '';
                                } else {
                                    const num = parseInt(quartosRow);
                                    qVal = (!isNaN(num) && num >= 4) ? '4+' : String(quartosRow);
                                }
                                
                                const isVenda = (row.OFERTA_VENDA === 'VENDIDOS' || row.OFERTA_VENDA === 'VENDIDOS - LANCADOS E VENDIDOS');
                                return bairroRow === bairro && qVal === q && isVenda;
                            });
                            
                            const ofertasData = filteredCrossData.filter(function(row) {
                                const bairroRow = row.BAIRRO || '';
                                let quartosRow = row.QTD_QUARTOS;
                                let qVal;
                                if (quartosRow === null || quartosRow === undefined || quartosRow === '') {
                                ...
                                const isOferta = (row.OFERTA_VENDA === 'OFERTADOS DISPONIVEIS' || row.OFERTA_VENDA === 'OFERTADOS LANCAMENTOS');
                                return bairroRow === bairro && qVal === q && isOferta;
                            });
                            
                            // Somar quantidades
                            const vendas = vendasData.reduce(function(sum, row) {
                                return sum + (parseFloat(row.QUANTIDADE) || 0);
                            }, 0);
                            
                            const ofertas = ofertasData.reduce(function(sum, row) {
                                return sum + (parseFloat(row.QUANTIDADE) || 0);
                            }, 0);
                            ...
                            bairroVendas += vendas;
                            bairroOfertas += ofertas;
                            
                            td.textContent = ivv.toFixed(1) + '%';
                            td.style.textAlign = 'center';
                            
                            // Aplicar cor baseada no valor do IVV
                            if (ivv >= 70) {
                                td.style.backgroundColor = '#d4edda'; // Verde claro
                                td.style.color = '#155724';
                            } else if (ivv >= 40) {
                                td.style.backgroundColor = '#fff3cd'; // Amarelo claro
                                td.style.color = '#856404';
                            } else if (ivv > 0) {
                                td.style.backgroundColor = '#f8d7da'; // Vermelho claro
                        ...
                        // Coluna total: IVV médio do bairro
                        const tdTotal = document.createElement('td');
                        const ivvTotal = bairroOfertas > 0 ? (bairroVendas / bairroOfertas) * 100 : 0;
                        tdTotal.textContent = ivvTotal.toFixed(1) + '%';
                        tdTotal.style.textAlign = 'center';
                        tdTotal.style.fontWeight = '600';
                        
                        // Aplicar cor baseada no IVV total
                        if (ivvTotal >= 70) {
                            tdTotal.style.backgroundColor = '#d4edda';
                            tdTotal.style.color = '#155724';
                        } else if (ivvTotal >= 40) {
                            tdTotal.style.backgroundColor = '#fff3cd';
                            tdTotal.style.color = '#856404';
                        } else if (ivvTotal > 0) {
                            tdTotal.style.backgroundColor = '#f8d7da';
                            tdTotal.style.color = '#721c24';
                        } else {
                            tdTotal.style.backgroundColor = '#f8f9fa';
                            tdTotal.style.color = '#6c757d';
                        }
                        
                        row.appendChild(tdTotal);
                        tbody.appendChild(row);
                        
                        totalVendas += bairroVendas;
                        totalOfertas += bairroOfertas;
                    });
                    ...
                    rooms.forEach(function(q) {
                        ...
                        // Calcular totais por quartos
                        const vendasQuarto = filteredCrossData.filter(function(row) {
                            let quartosRow = row.QTD_QUARTOS;
                            let qVal;
                            if (quartosRow === null || quartosRow === undefined || quartosRow === '') {
                                qVal = '';
                            ...
                            const isVenda = (row.OFERTA_VENDA === 'VENDIDOS' || row.OFERTA_VENDA === 'VENDIDOS - LANCADOS E VENDIDOS');
                            return qVal === q && isVenda;
                        }).reduce(function(sum, row) {
                            return sum + (parseFloat(row.QUANTIDADE) || 0);
                        }, 0);
                        
                        const ofertasQuarto = filteredCrossData.filter(function(row) {
                            let quartosRow = row.QTD_QUARTOS;
                            let qVal;
                            if (quartosRow === null || quartosRow === undefined || quartosRow === '') {
                                qVal = '';
                            ...
                            const isOferta = (row.OFERTA_VENDA === 'OFERTADOS DISPONIVEIS' || row.OFERTA_VENDA === 'OFERTADOS LANCAMENTOS');
                            return qVal === q && isOferta;
                        }).reduce(function(sum, row) {
                            return sum + (parseFloat(row.QUANTIDADE) || 0);
                        }, 0);
                        
                        const ivvQuarto = ofertasQuarto > 0 ? (vendasQuarto / ofertasQuarto) * 100 : 0;
                        td.textContent = ivvQuarto.toFixed(1) + '%';
                        td.style.textAlign = 'center';
                    ...
                    // Total geral
                    const tdTotalGeral = document.createElement('td');
                    const ivvGeral = totalOfertas > 0 ? (totalVendas / totalOfertas) * 100 : 0;
                    tdTotalGeral.textContent = ivvGeral.toFixed(1) + '%';
                    tdTotalGeral.style.textAlign = 'center';
                    tdTotalGeral.style.fontWeight = '700';
                    tdTotalGeral.style.fontSize = '14px';
                    totalRow.appendChild(tdTotalGeral);
                    ...
                    legendDiv.innerHTML = '<strong>📊 IVV (Índice de Velocidade de Vendas):</strong> Calcula a relação percentual entre vendas realizadas e unidades ofertadas por região.<br>' +
                        ...
                        '<strong>Interpretação:</strong> <span style="color:#155724;">■</span> Acima de 70% = Excelente | <span style="color:#856404;">■</span> 40-70% = Bom | <span style="color:#721c24;">■</span> Abaixo de 40% = Atenção';
